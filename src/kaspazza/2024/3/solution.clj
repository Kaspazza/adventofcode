...
(def corrupted-input "xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))")

(def corrupted-input2 "xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))")
...
(defn find-mul-fns "Finds mul functions in corrupted memory" [s] (re-seq #"mul\((\d+,\d+)\)" s))
